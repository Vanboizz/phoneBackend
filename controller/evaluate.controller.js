...
const addEvaluate = async (req, res) => {
  try {
    ...
    const { idproducts, starnumber, review, images, performance, batterylife, cameraquantity } =
      req.body;
    const idusers = req.auth.id; // Assuming this is a valid way to get the user's ID

    // Your SQL queries
    const queryGetEvaluate =
      'SELECT * FROM detailinvoice WHERE idproducts = ? AND idiv IN (SELECT idiv FROM invoice WHERE idusers = ?)';
    const queryAddEvaluate =
      'INSERT INTO evaluate(idusers, idproducts, evaluateday, starnumber, review, images, performance, batterylife, cameraquantity) VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?)';
...
const getStatisticsOfReview = (req, res) => {
  try {
    const idproducts = req.params.idproducts;
    const queryGetEvaluate =
      'SELECT idevaluate,starnumber,performance,batterylife,cameraquantity FROM evaluate WHERE idproducts = ?';
...
const getAllEvaluate = (req, res) => {
  try {
    let queryGetRatings =
      'SELECT e.idproducts, SUM(e.starRating * e.ratingCount) AS totalStars, SUM(e.ratingCount) AS totalRatings ' +
      "FROM (SELECT idproducts, JSON_EXTRACT(starnumber, '$.number') AS starRating, COUNT(*) AS ratingCount " +
      "FROM evaluate GROUP BY idproducts, JSON_EXTRACT(starnumber, '$.number')) AS e " +
      'GROUP BY e.idproducts ORDER BY e.idproducts';

    db.connection.query(queryGetRatings, (error, results) => {
      if (error) throw error;

      const ratingsMap = {};
      results.forEach((row) => {
        if (!ratingsMap[row.idproducts]) {
          ratingsMap[row.idproducts] = { totalStars: 0, totalRatings: 0 };
        }
        ratingsMap[row.idproducts].totalStars += row.totalStars;
        ratingsMap[row.idproducts].totalRatings += row.totalRatings;
      });

      const ratingsArray = Object.keys(ratingsMap).map((idproducts) => {
        const averageRating = ratingsMap[idproducts].totalRatings
          ? ratingsMap[idproducts].totalStars / ratingsMap[idproducts].totalRatings
          : 0;
        return {
          idproducts,
          averageRating,
      ...
      res.status(200).json({ data: ratingsArray });
    });
  } catch (error) {
    res.status(500).json({ error: 'Error Server.' });
  }
