...
const addComment = (req, res) => {
  ...
  if (!isParentComment) {
    ...
    db.connection.query(queryCheckParent, [parentId], (error, result) => {
      ...
      db.connection.query(
        queryAddComment,
        [idusers, idproducts, comment, parentId],
        (error, result) => {
          if (error) {
...
const getComment = (req, res) => {
  ...
  const querySelectComment =
    'select * from comments,users where idproducts = ? and comments.idusers = users.idusers ORDER BY commentday DESC';
